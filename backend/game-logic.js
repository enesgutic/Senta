...
function autoDrawIfNoMoves(game) {
  const p0CanMove = playerHasMove(game.players[0], game.centerPiles);
  const p1CanMove = playerHasMove(game.players[1], game.centerPiles);
  if (!p0CanMove && !p1CanMove) {
    for (let i = 0; i < 2; i++) {
      if (game.players[i].deck.length) {
        game.centerPiles[i].push(game.players[i].deck.pop());
      }
    }
  }
}
...
function sentaAction(game, playerId) {
  ...
  if (game.players[0].deck.length) game.centerPiles[0] = [game.players[0].deck.pop()];
  else game.centerPiles[0] = [];
  if (game.players[1].deck.length) game.centerPiles[1] = [game.players[1].deck.pop()];
  else game.centerPiles[1] = [];
...
// REVISED: votes by player index, not socket id
...
module.exports = { createGame, joinGame, playCard, sentaAction, rematch, playerForceDraw };
